...
```{r panel_1A, fig.width=19,fig.height=16}
...
data2<-as.data.frame(read.delim("Data/Figure1/Cohorts_figure1A.tab",colClasses = c("factor","factor","numeric","factor","numeric","numeric","numeric","numeric")))

#
# Set a number of 'empty bar' to add at the end of each Cohort
empty_bar=2
nObsType=5 #Set as number of divisions wanted for plotting (eg. 10%, 20%....100%)
data2$Cohort<-as.factor(data2$Cohort)
data2$Cohort <- factor(data2$Cohort,levels=c("ICGC","TCGA","ICI","Riaz et al","Hematological","Pediatric","Composite"))
data2$Subgroup <- factor(data2$Subgroup,levels=c("ICGC","TCGA","MSKCC-IMPACT","Pender et al","Miao et al","Braun et al","Liu et al","Hugo et al","Riaz et al","Hematological","Pediatric","Composite"))
data2$Tumor_type<-factor(data2$Tumor_type,levels=names(table(data2$Tumor_type)))


# Get the name and the y position of each label for each bar
label_data= data2 %>% group_by(id, Tumor_type) %>% dplyr::summarize(tot=sum(n_corrected),Samples=total)
label_data$lab<-paste0(label_data$Tumor_type,"(n=",label_data$Samples,")")
 label_data<-label_data[!duplicated(label_data$id),]
...
# prepare a data frame for base lines, to nae the cohorts
base_data=data2 %>% 
  group_by(Cohort) %>% 
...
grid_data$end =grid_data$end +.5
grid_data$start =grid_data$start +.5
grid_data=grid_data[-1,]

 ###Edit for lines of cohorts...
base_data$start<-base_data$start+.5
base_data$end<-base_data$end+.5
##
base_data$lab<-paste0(base_data$Cohort,"\nn=",c("2775","9264","2261","68","3859","1051","31353"))
##
base_data_lines<-base_data
base_data_lines$start[]<-base_data_lines$start[]-1;base_data_lines$end[]<-base_data_lines$end[]+1
...
##Little edit
base_data$lab[4]<-"Riaz et al; n=68"

#Color fill by subgroup, remain the same as cohorts exept for ICI
cols2<-c("dodgerblue4","firebrick1","peru","chocolate3","chocolate2","chocolate1","orange1","salmon","seagreen4","sienna","darkviolet","black")
names(cols2)<-names(table(data2$Subgroup))


p_cohorts1 <-ggplot(data2) +      
    # Add the stacked bar
    geom_bar(aes(x=as.factor(id), y=n_corrected, fill=Subgroup), stat="identity", alpha=0.5) +scale_fill_manual(values = cols2,name="Subgroup")+
    ...
    annotate("text", x = rep(max(data2$id),5), y = c(0, 1, 2, 3, 4), label = expression("0",10^1,10^2,10^3,10^4) , color="red", size=2 , angle=0, fontface="bold", hjust=.5) +
    ...
    geom_text(data=label_data, aes(x=id, y=tot, label=lab, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=2.5, angle= label_data$angle, inherit.aes = FALSE ) +
    
      # Add base line information
     # Add base line information
    geom_segment(data=base_data_lines, aes(x = start, y = -.1, xend = end, yend = -.1), colour = c("dodgerblue4","firebrick1","chocolate1","seagreen4","sienna","darkviolet","black"), alpha=0.8, size=0.6 , inherit.aes = FALSE )  +
    geom_text(data=base_data, aes(x = title, y = -.3, label=lab), hjust=c(0.5,.75,1.,.1,.1,.4,0),vjust=c(1,0,0,.5,.33,-.5,0.5), angle=c(0,0,0,60,62,0,0),colour = c("dodgerblue4","firebrick1","chocolate1","seagreen4","sienna","darkviolet","black"), alpha=0.8, size=4, fontface="bold", inherit.aes = FALSE)



##Legend..

p1_legend <- extract_gglegend(ggplot(data2) +      
    # Add the stacked bar
    geom_bar(aes(x=as.factor(id), y=n_corrected, fill=Subgroup), stat="identity", alpha=0.5) +scale_fill_manual(values = cols2,name="Cohort group")+ guides(fill = guide_legend(ncol = 2,title.theme = element_text(face="bold",hjust = 0.5))))

xbp_grob <- ggplotGrob(p1_legend$legend)

p_cohorts2<-plot_grid(p_cohorts1+ theme(plot.margin = unit(c(-15,-15,-15,0), "lines")),p1_legend$legend+ theme(plot.margin = unit(c(-2,-15,-2,-30), "lines")),rel_heights=c(1,.02),ncol=2,nrow=1)



###Internal Pie chart...
pie_dat<-data.frame(Cohort=names(table(data2$Cohort)),values=c(2775,9264,2261,68,3859,1051,31353))

pie_dat$prop<-pie_dat$values/sum(pie_dat$values)

# Add label position
pie_dat <- pie_dat %>%  mutate(lab.ypos = cumsum(prop) - 0.5*prop)
pie_dat
pie_dat$prop<-round(pie_dat$prop,digits = 4)
pie_dat$prop_labl<-paste0(pie_dat$prop*100,"%")

##Reorder

cols<-c("dodgerblue4","firebrick1","chocolate1","seagreen4","sienna","darkviolet","black")
 names(cols)<-pie_dat$Cohort
 pie_dat$Cohort<-factor(pie_dat$Cohort,levels=rev(names(cols)))
            
p_pie<-ggplot(pie_dat, aes(x = 2, y = prop, fill = Cohort)) +
    geom_bar(width = 1, stat = "identity", color = "white") +
    coord_polar("y", start = 1)+
    geom_text_repel(aes(y = lab.ypos-.005, label = prop_labl,x = 1.9), color = "white", 
                    segment.size = .4,nudge_x = .05, 
                    show.legend = FALSE,size = 7)+
    xlim(.2,2.5)+
    theme_void()+scale_fill_manual(values = cols,name="Cohort group")+theme(legend.title = element_text(size = 9,face="bold"),legend.key.size = unit(0.5, "cm"),legend.text = element_text(size = 9))+ guides(fill = guide_legend(ncol = 2)) +theme(legend.position = "none")

p_pie<-p_pie+annotate("text", label ="50,631\nTumors",  x=0.25, y=0.25, fontface =2,size=25)
...
```{r panel_1A2, fig.width=14,fig.height=12}
p_pie
```
...
```{r calculations_fig1B,echo=F,eval=F}
...
already_done<-read.table("/media/user/seagate_ICM/AICDA_analysis/ICGC/AlreadyDone_TumorBarcodes_.tsv", quote="\"", comment.char="")
...
pcawg_sample_sheet <- read.delim("/media/user/seagate_ICM/AICDA_analysis/ICGC/pcawg_sample_sheet.tsv")
...
```{r panel_1B,fig.height=12, fig.width=14}
library(ggpubr)
library(reshape2)
##Import data
data_dir<-"/media/user/seagate_ICM/AICDA_analysis/Github_repository/" #Set to where you downloaded the data
setwd(data_dir)
ICGC_clinial_n3110<-read.delim("Data/Figure1/ICGC_data_figure1B.tab")
...
```{r calculations_fig1C,echo=F,eval=F}
library(Palimpsest)
library(maftools)
...
```{r panel_1C_cals, eval=T,echo=T,fig.show="hide",results="hide"}
...
signature_proportions_semi_icgc2<-as.data.frame(read.delim("Data/Figure1/ICGC_data_figure1C.tab"))
signature_mutations_median_semi_icgc2<-as.data.frame(read.delim("Data/Figure1/ICGC_data_figure1C_median.tab"))
...
```{r panel_1D_cals,eval=T,echo=T,fig.show="hide", results="hide"}
##LOad manual function for corplot 
...
load("Data/Figure1/ICGC_data_figure1D.RData")
##
p_signatures_semideconv_icgc<-list();total_n_icgc<-rep(1,35)
for (i in 1:35) {
  order<-NULL
  if (is.null(SBS_signatures_exp_pancancer_NO_AICDA_icgc[[i]]$sig_props$AICDA_canonical)=="FALSE") {
    order<-order(colSums(t(SBS_signatures_exp_pancancer_NO_AICDA_icgc[[i]]$sig_props$AICDA_canonical)), decreasing = T)#Order according to highest AICDA canonical proportion (if present)
  }
  order_sigs<-colnames(SBS_signatures_exp_pancancer_NO_AICDA_icgc[[i]]$sig_props)
  if (is.null(SBS_signatures_exp_pancancer_NO_AICDA_icgc[[i]]$sig_props$SBS9)=="FALSE") {
  ...
  p_signatures_semideconv_icgc[[i]]<-signature_exposure_plot2(regression_line = "AICDA_canonical",order = order,signature_colours = sig_cols,mutSign_nums =   SBS_signatures_exp_pancancer_NO_AICDA_icgc[[i]]$sig_nums[,order_sigs],mutSign_props =SBS_signatures_exp_pancancer_NO_AICDA_icgc[[i]]$sig_props[,order_sigs] )+ ggtitle(paste0("Mutational Signature exposure in ",names(SBS_signatures_exp_pancancer_NO_AICDA_icgc[i])," (n = ",nrow(SBS_signatures_exp_pancancer_NO_AICDA_icgc[[i]]$sig_nums),")"))+theme(axis.text.x=element_blank())+xlab(paste0(names(SBS_signatures_exp_pancancer_NO_AICDA_icgc[i])," (n = ",nrow(SBS_signatures_exp_pancancer_NO_AICDA_icgc[[i]]$sig_nums),")")) #Remove samples names
  total_n_icgc[i]<-nrow(SBS_signatures_exp_pancancer_NO_AICDA_icgc[[i]]$sig_nums)
}
names(total_n_icgc)<-icgc_tumorType


###Arrange together using grid
gb_psignatures_semideconv_icgc<-list()
for (i in 1:35) {
  if (i%in%seq(1,35,by=5)) {
    gb_psignatures_semideconv_icgc[[i]] = grid.grabExpr({
      print(p_signatures_semideconv_icgc[[i]]+theme(legend.position='none')+theme(plot.title = element_blank(), axis.ticks.x=element_blank(),axis.line.x=element_blank())+scale_y_continuous(expand = c(0,0)))
    })
  }else{
    if(i %in% setdiff(1:35,seq(1,35,by=5))){
      gb_psignatures_semideconv_icgc[[i]] = grid.grabExpr({
        print(p_signatures_semideconv_icgc[[i]]+theme(legend.position='none')+ theme(plot.title = element_blank(), axis.ticks.x=element_blank(),axis.line.x=element_blank(), axis.ticks.y=element_blank(),axis.line.y=element_blank(),axis.title.y = element_blank(),axis.text.y = element_blank())+scale_y_continuous(expand = c(0,0)) )
      })}
  }
}
...
###Arrange by 5
gb_psignatures_semideconv_by5_icgc<-list()
...
for (j in 0:6) {
  ...
  for (i in 0:5) {
    ...
    k<-ifelse(i==5,yes = 30,no = k)
    grid.draw(gb_psignatures_semideconv_icgc[[i+k]])
    popViewport()
  }
  gb_psignatures_semideconv_by5_icgc[[j+1]]<- recordPlot()
}
...
library(grid)
#grid.echo()
gb_psignatures_semideconv_by5_icgc_2<-list()
gb_psignatures_semideconv_by5_icgc[[1]]
gb_psignatures_semideconv_by5_icgc_2[[1]] <- grid.grab()
gb_psignatures_semideconv_by5_icgc[[2]]
gb_psignatures_semideconv_by5_icgc_2[[2]] <- grid.grab()
gb_psignatures_semideconv_by5_icgc[[3]]
gb_psignatures_semideconv_by5_icgc_2[[3]] <- grid.grab()
gb_psignatures_semideconv_by5_icgc[[4]]
gb_psignatures_semideconv_by5_icgc_2[[4]] <- grid.grab()
gb_psignatures_semideconv_by5_icgc[[5]]
gb_psignatures_semideconv_by5_icgc_2[[5]] <- grid.grab()
gb_psignatures_semideconv_by5_icgc[[6]]
gb_psignatures_semideconv_by5_icgc_2[[6]] <- grid.grab()
gb_psignatures_semideconv_by5_icgc[[7]]
gb_psignatures_semideconv_by5_icgc_2[[7]] <- grid.grab()



#grab legend
##for legend
sig_cols_plot_semi_icgc<-"SBS1"

for (i in 1:35) {
  sig_cols_plot_semi_icgc<-c(sig_cols_plot_semi_icgc,colnames(SBS_signatures_exp_pancancer_NO_AICDA_icgc[[i]]$sig_props))
}

sig_cols_plot_semi_icgc<-unique(sig_cols_plot_semi_icgc);sig_cols_plot_semi_icgc<-sig_cols[sig_cols_plot_semi_icgc]
sig_cols_plot_semi_icgc<-sig_cols_plot_semi_icgc[order(factor(names(sig_cols_plot_semi_icgc), levels = names(sig_cols)))]#reorder

lgd_semi_icgc = Legend(labels = names(sig_cols_plot_semi_icgc), legend_gp = gpar(fill = sig_cols_plot_semi_icgc), 
...
grid.newpage()
grid.draw( editGrob(gb_psignatures_semideconv_by5_icgc_2[[1]], vp=viewport(y = 0.87, x=0.5,height = .13, width = 1, just = c("bottom")), gp=gpar(fontsize=10)))
grid.draw( editGrob(gb_psignatures_semideconv_by5_icgc_2[[2]], vp=viewport(y = 0.74, x=0.5,height = .13, width = 1, just = c("bottom")), gp=gpar(fontsize=10)))
grid.draw( editGrob(gb_psignatures_semideconv_by5_icgc_2[[3]], vp=viewport(y = 0.61, x=0.5,height = .13, width = 1, just = c("bottom")), gp=gpar(fontsize=10)))
grid.draw( editGrob(gb_psignatures_semideconv_by5_icgc_2[[4]], vp=viewport(y = 0.48, x=0.5,height = .13, width = 1, just = c("bottom")), gp=gpar(fontsize=10)))
grid.draw( editGrob(gb_psignatures_semideconv_by5_icgc_2[[5]], vp=viewport(y = 0.35, x=0.5,height = .13, width = 1, just = c("bottom")), gp=gpar(fontsize=10)))
grid.draw( editGrob(gb_psignatures_semideconv_by5_icgc_2[[6]], vp=viewport(y = 0.22, x=0.5,height = .13, width = 1, just = c("bottom")), gp=gpar(fontsize=10)))
grid.draw( editGrob(gb_psignatures_semideconv_by5_icgc_2[[7]], vp=viewport(y = 0.09, x=0.5,height = .13, width = 1, just = c("bottom")), gp=gpar(fontsize=10)))

pushViewport(viewport(  y = 0.06,x=.5,height = .15,width =.8))
grid.draw(lgd_semi_icgc)


p_pancancer_signatures_semideconv_icgc<-recordPlot()



```

```{r panel_1D,fig.height=18, fig.width=20}
p_pancancer_signatures_semideconv_icgc
```
